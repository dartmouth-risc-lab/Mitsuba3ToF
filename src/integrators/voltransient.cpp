...
class VolTransientPathIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
...
public:
    ...
    VolTransientPathIntegrator(const Properties &props) : Base(props) {
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene,
        ...
        dr::Loop<Mask> loop("Volpath integrator",
                            ...
                            specular_chain, valid_ray, sampler, path_length);
        ...
        while (loop(active)) {
            ...
            if (dr::any_or<true>(active_medium)) {
                ...
                path_length += dr::select(act_medium_scatter, mei.t * eta, 0);
            ...
            if (dr::any_or<true>(act_medium_scatter)) {
                ...
                if (dr::any_or<true>(active_e)) {
                    ...
                    Spectrum L = throughput * phase_val * emitted * mis_weight(ds.pdf, dr::select(ds.delta, 0.f, phase_val));
                    dr::masked(result, active_e) += L;
            ...
            if (dr::any_or<true>(intersect))
                dr::masked(si, intersect) = scene->ray_intersect(ray, intersect);
            path_length += dr::select(intersect && si.is_valid(), si.t * eta, 0);
...
MI_IMPLEMENT_CLASS_VARIANT(VolTransientPathIntegrator, MonteCarloIntegrator);
MI_EXPORT_PLUGIN(VolTransientPathIntegrator, "Volumetric Path Tracer integrator");
