...
class FMCWPathIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
public:
    MI_IMPORT_BASE(MonteCarloIntegrator, m_max_depth, m_rr_depth, m_hide_emitters)
    MI_IMPORT_TYPES(Scene, Sampler, Medium, Emitter, EmitterPtr, BSDF, BSDFPtr)

    FMCWPathIntegrator(const Properties &props) : Base(props) {
        m_f_c = props.get<ScalarFloat>("f_c", 77);  // GHz (1e9)
        if(props.has_property("wavelength")){
            ScalarFloat wavelength = props.get<ScalarFloat>("wavelength", 1550); // nm (1e-9)
            m_f_c = 3e8 / wavelength;
        }
        m_B = props.get<ScalarFloat>("B", 0.15);  // GHz (1e9)
        m_T = props.get<ScalarFloat>("T", 7.33);  // micro second (1e-6)
        m_time = props.get<ScalarFloat>("time", 0); // micro second (1e-6)
        m_R_min = 2 * props.get<ScalarFloat>("R_min", 0); // m
        bool invert = props.get<bool>("invert", false);
        if(invert){
            m_f_c += m_B;
            m_B = -m_B;
        }
    ...
    std::pair<Spectrum, Bool> sample(const Scene *scene,
        ...
        while (loop(active)) {
            ...
            if (dr::any_or<true>(dr::neq(si.emitter(scene), nullptr))) {
                ...
                Float modulation_weight = get_fmcw_weight(m_time, path_length);

                // Accumulate, being careful with polarization (see spec_fma)
                result = spec_fma(
                    throughput,
                    ds.emitter->eval(si, prev_bsdf_pdf > 0.f) * mis_bsdf * modulation_weight,
                    result);
            }
            ...
            if (dr::any_or<true>(active_em)) {
                ...
                Float em_path_length = path_length + ds.dist * eta;
                Float modulation_weight = get_fmcw_weight(m_time, em_path_length);

                // Accumulate, being careful with polarization (see spec_fma)
                result[active_em] = spec_fma(
                    throughput, bsdf_val * em_weight * mis_em * modulation_weight, result);
            }
    ...
    std::string to_string() const override {
        return tfm::format("FMCWPathIntegrator[\n"
            "  max_depth = %u,\n"
            "  rr_depth = %u\n"
...
MI_IMPLEMENT_CLASS_VARIANT(FMCWPathIntegrator, MonteCarloIntegrator)
MI_EXPORT_PLUGIN(FMCWPathIntegrator, "FMCW Path Tracer integrator");
